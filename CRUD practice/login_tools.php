...
function validate($link, $email = "", $pwd = "") {
    ...
    if (empty($errors)) { //if the errors array is empty...
        $q = "SELECT user_id, first_name, last_name FROM users WHERE email='$e' AND pass='$p'"; //Create an SQL query to fetch relevant data (NOTE: variable names have to be in single quotes here)
        $r = mysqli_query ($link, $q); //Run query through $link connection variable
        if (@mysqli_num_rows($r) == 1) { //If the query returns exactly one row (implying the email address and password exist in the database)...
